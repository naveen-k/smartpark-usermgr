...
const Service = function(configuration) {
  ...
  if(!config) {
    ...
    config.db.keyspace = process.env.DB_KEYSPACE || 'smartpark-user';
  }

  log.debug('Service Initialized', 'UserService()');
  log.debug(config, 'UserService.config');
  ...
  var updateItem = function(args) {
    ...
    let changed = new Model.User(args);
  ...
  // Get List from the Database for all User
  var listItems = function(params) {
    ...
    Table.find(filter,{raw:true, allow_filtering: true}, function(err, rows){
      ...
      for(var row of rows) {
          ...
          log.debug("listItems :", row);
        pageResult.Users.push(new Model.User(row));
      }
  ...
  var sendError = function(error, message) {
    ...
    log.error(error, 'UserService.sendError');
  ...
  var openConnection = function(eventHandler, args) {
    log.debug('DB Connection Open', 'UserService.openConnection()');
    let errorMsg = 'DB Connection Failure';

    db.connect(config.db, function(err, db) {
      if(err || db.instance.User === undefined) {return self.emit('send-error', err, errorMsg);}
      ...
      Table.find({}, function(err, result) {
        ...
        if(result.length === 0) {
          log.debug('Seeding the Database', 'UserService.openConnection()');
          let data = require('./seed.json');
          let count = 0;

          for (var value of data) {
          log.debug(result, 'UserService seeding data +++++++',value);

            let dto = new Model.User(value);
            log.debug(result, 'UserService seeding data ------',dto.toJson());
